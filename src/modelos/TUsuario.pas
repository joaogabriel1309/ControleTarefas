...
type
  ...
  public
    ...
    constructor Create(const Codigo: Integer; const Nome: string; const Direito: string; const Senha: string); overload;
...
constructor Usuario.Create(const Codigo: Integer; const Nome: string; const Direito: string; const Senha: string);
begin
  inherited Create;
  FCodigo := Codigo;
  FNome := Nome;
...
begin
  ...
  try
    ...
    Query.Close;
    Query.SQL.Clear;
    Query.SQL.Add('INSERT INTO usuario (codigo, nome, direito, senha) ' +
                  'VALUES (:codigo, :nome, :direito, :senha) ' +
                  'ON DUPLICATE KEY UPDATE nome = :nome, direito = :direito, senha = :senha');
...
begin
  ...
  if BuscarPorCodigo(FCodigo) <> nil then
  begin
    Query := TFDQuery.Create(nil);
    try
      Query.Connection := Conexao.Tconexao.GetConexao();
...
class function Usuario.PegaMaxCodigo: Integer;
var
  Query: TFDQuery;
begin
  Result := 0;
  ...
  try
    ...
    Query.Close;
    Query.SQL.Clear;
    Query.SQL.Add('SELECT MAX(codigo) AS MaxCodigo FROM usuario');
    Query.Open;

    if not Query.IsEmpty then
    begin
      Result := Query.FieldByName('MaxCodigo').AsInteger;
    end;
