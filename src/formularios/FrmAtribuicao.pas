...
type
  TTFrmAtribuicao = class(TForm)
    ...
    procedure ImageTarefaClick(Sender: TObject);
  private
...
uses TUsuario, TTarefa, TUsuarioTarefa, frmPesquisaUsuario, frmPesquisaTarefa;
...
procedure TTFrmAtribuicao.FormClick(Sender: TObject);
var
  frmPesquisa : TTfrmPesquisaTarefa;
begin
  frmPesquisa := TTfrmPesquisaTarefa.Create(self);
  try
    frmPesquisa.ShowModal;
    if not (frmPesquisa.CodPesquisa = 0) then
      EditCodTarefa.Text := IntToStr(frmPesquisa.CodPesquisa);
    EditCodTarefaExit(EditCodTarefa);
  finally
    frmPesquisa.Free;
  end;
end;

procedure TTFrmAtribuicao.ImageTarefaClick(Sender: TObject);
var
  frmPesquisa : TTfrmPesquisaTarefa;
begin
  frmPesquisa := TTfrmPesquisaTarefa.Create(self);
...
procedure TTFrmAtribuicao.ImageUsuarioClick(Sender: TObject);
var
